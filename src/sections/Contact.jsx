    ...
    emailjs
      ...
      .then(
        ...
        (error) => {
          setLoading(false);
          console.log(error);
          alert(t("form_questions.email_error_reponse_message"));
        }
      );
  };
        ...
        <div className="flex w-full max-w-lg bg-[#32303a] sm:p-8 p-6 rounded-xl text-white">
          ...
          >
            <label className="flex flex-col">
              <span className="text-white font-medium mb-2">{t("form_questions.name")}</span>
              <input
                type="text"
                name="name"
                value={form.name}
            ...
            <label className="flex flex-col">
              <span className="text-white font-medium mb-2">{t("form_questions.email")}</span>
              <input
                type="email"
                name="email"
                value={form.email}
            ...
            <label className="flex flex-col">
              <span className="text-white font-medium mb-2">{t("form_questions.message")}</span>
              <textarea
                name="message"
                value={form.message}
                onChange={handleChange}
                ...
                placeholder={t("form_questions.message_ph")}
              />
            </label>
            ...
            >
              {loading ? t("form_questions.sending") : t("form_questions.send")}
            </motion.button>
          </form>
        </div>
      </motion.div>
